...
from convertWindMat_2D import *  # noqa: F403

errPath = os.path.join(os.getcwd(), 'workflow.err')  # noqa: N816, PTH109, PTH118


def main2D(aimName, evtName, getRV):  # noqa: C901, N803, D103, PLR0915
    """
    This is an edited version of the original 3D script. 

    """

    #print('this goes into a 2D_MSA from 2D file.')
    individual_ramp_duration = 120 # seconds
    ...
    T_full = T_full + 2*individual_ramp_duration # add time for a ramp up + a ramp down
    ...
    if case == 'timeHistory':
        ...
        Fx = np.array([]).reshape([0,0])
        Fy = np.array([]).reshape([0,0])
        Tz = np.array([]).reshape([0,0])

        enabled = []
        ######### Populate arrays when possible
        if 'Fx' in data : 
        ...
        t = data['t']  # noqa: F841
        N = max(Fx.shape[1],Fy.shape[1])  # noqa: N806
        nfloors = max(Fx.shape[0],Fy.shape[0])
    ...
    if case == 'timeHistory' : 
        options = ['Fx','Fy', 'Tz']
        #options = ['Fy','Tz']        
        axes = [x for x in options if x in enabled]
        ndir = len(axes)
    elif case == 'PODmodes' : 
        nfloors_GI = aim_data['GeneralInformation']['NumberOfStories']
        # Take any of the previously calculated variables to count the number of directions :
        ndir = int(V_imag.shape[0]/nfloors_GI)
    #######################################################
    ...
    ####################################################################################################################################
    static_pres = np.array([])

    if 'Fx' in axes :
        static_pres_x = np.ones((nfloors, 1, 1)) * (0.5 * air_dens * vRef**2 * H_full * B_full)
        static_pres = np.vstack([static_pres,static_pres_x]) if static_pres.size else static_pres_x
    if 'Fy' in axes :
        static_pres_y = np.ones((nfloors, 1, 1)) * (0.5 * air_dens * vRef**2 * H_full * D_full)
        static_pres = np.vstack([static_pres,static_pres_y]) if static_pres.size else static_pres_y
    if 'Tz' in axes :
        static_pres_z = np.ones((nfloors, 1, 1))* (0.5 * air_dens * vRef**2 * H_full * MaxD_full**2 / 2)
        static_pres = np.vstack([static_pres,static_pres_z]) if static_pres.size else static_pres_z
    ...
    evtInfo['numSteps'] = tvec.shape[0] + 1 ## prepend a timestep of 0.
    ...
    for id in range(id_timeseries):  # noqa: A001
        ...
        base_sim = F_sim[(cur_dof) * nfloors + cur_floor, :, 0] # includes time for two ramps already (120 seconds each)
        nSteps = math.ceil(individual_ramp_duration/my_ts['dT']) # Identify the number of steps this represents       
        ramp = np.linspace(0,1,nSteps)[:-1] # create the ramp factors (up/down)
        inv_ramp = ramp[::-1]                   
        full_scale_wind = np.ones(len(base_sim)-2*len(ramp)) # Get factors for the unmodified forces
        ramp_factors = np.concatenate([ramp,full_scale_wind,inv_ramp]) # Final array describing ramp factors from start to end
        ramped_F_sim = base_sim*ramp_factors # Modify the forces
        my_ts['data'] = ramped_F_sim.tolist()
    ...
    plt.plot(tvec, F_sim[9 , :, 0] ,lw=1)
    if case == "timeHistory":
        t_sc = np.array(t)[0,:] * ms * (vRef / V_H) # scale wind tunnel time series to compare
        plt.plot(t_sc, (V_H / vRef) ** 2 * Fx_full[9 ,: ] ,lw=1)
    plt.xlabel('t(s)')
    plt.ylabel('Fx (kip)')
    plt.title('Force - Floor = 10 - Full Scale')
    plt.show()

    plt.plot(tvec, F_sim[34,:, 0] / 1000)
    if case == "timeHistory":
...
def learn_CPSD(  # noqa: N802, D103, PLR0913
    ...
    # Execute concatenations only if the arrays were supplied :
    ...
    comp_CFmean = np.array([])
    norm_all = np.array([])
    Components = np.array([])

    for arr in [CFx_mean,CFy_mean,CTz_mean] :
        if len(arr) != 0: 
            comp_CFmean = np.concatenate([comp_CFmean, arr])

    for arr in [xnorm,ynorm,tornorm] :
        if len(arr) != 0 :
            norm_all = np.concatenate([norm_all,arr]) if norm_all.size else arr

    for arr in [CFx_norm,CFy_norm,CTz_norm] : 
        if len(arr) != 0 :
            Components = np.vstack([Components, arr]) if Components.size else arr
